		...
		<div class="page-group">
								...
								<div id="lpxq_div" style="position: absolute; width: 100%; text-align: center; display: none;">
									<span style="display: inline-block">
										<span class="lpxq_span" id="lpxq_span">
											精美定制手机支架</br>请选择礼品领取门店</br>并在提示时间段内到店领取</br>过期未领，礼品将返还礼品池
										</span>
									</span>
			...
			<div id="city_popup" class="popup">
				<div class="bar bar-footer" style="margin: 0; padding: 0; display: flex; background: #f7cd6e; align-items: center; justify-content: center; text-align: center;">
					<span style="background: #090e28; padding: 0.2rem 1.5rem; display: inline-block; font-size:0.75rem; color: #f7cd6e" onclick="close_popup('city_popup')">取消</span>
				</div>
				<div class="content" style="background: url(<%=PropKit.get("front_res_prefix") %>wxcj/img/gsjj_bg.jpg); background-size: cover; width: 100%; height: 100%; position: relative; padding-bottom: 55px;">
		...
		<script type="application/javascript">
			...
			function qrlq() {
				...
				if ($.isEmptyObject(shopid)) {
					...
					$.post('/<%=PropKit.get("project_name") %>/wxcj/jinjie1_qrdh_json', params, function (returnJson){
		    			var rescode = returnJson.rescode;
		    			var rescnt = returnJson.rescnt;
		    			if (rescode == '0000') {
		    				window.location.href = '/<%=PropKit.get("project_name") %>/wxcj/jinjie1_lpmm';
		    			} else {
		    				window.location.href = '/<%=PropKit.get("project_name") %>/wxcj/jinjie1_ljsb';
		    			}
		    		}, "json");
				}
