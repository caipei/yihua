...
<form action="/<%=PropKit.get("project_name") %>/admin/leysen/zxdt/submit" id="leysen_zxdt_addform" data-toggle="validate" data-alertmsg="false">
	<table class="table table-condensed table-hover" width="100%">
        <tbody>
        <tr>
	        <td>
	            <label for="title" class="control-label x100">标题：</label>
	            <input type="text" name="title" id="title" value="" placeholder="标题" size="80">
	        </td>
        </tr>
        <tr>
	        <td style="display: flex; align-items: center;">
	            ...
	            <div style="display: inline-block; vertical-align: middle; flex: 1; margin-left: 4px;">
					<span id="leysen_zxdt_add_img_upload_span" class="img_thumbnail_add_span">
						<img class="img_upload" id="leysen_zxdt_add_img_upload" src="/<%=PropKit.get("project_name") %>/res/images/img_add.png">
					</span>
					<span id="leysen_zxdt_add_img_imgs_div">
					</span>
...
$(document).ready(function(){
	...
	$.CurrentNavtab.find("#leysen_zxdt_submit").click(function() {
		...
		var imgpath = '';
		$.CurrentNavtab.find('#leysen_zxdt_add_img_imgs_div img[name="img_thumbnail"]').each(function() {
			imgpath = $(this).attr('src');
		});
		$.CurrentNavtab.find('#imgpath').val(imgpath);
		
		$.CurrentNavtab.find("#leysen_zxdt_addform").submit();
	});
	
	uploadimg('leysen_zxdt_add_', 1);
});
