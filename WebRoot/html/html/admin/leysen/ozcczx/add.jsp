...
<form action="/<%=PropKit.get("project_name") %>/admin/leysen/ozcczx/submit" id="leysen_ozcczx_addform" data-toggle="validate" data-alertmsg="false">
        ...
        <tr>
	        <td style="display: flex; align-items: center;">
	            ...
	            <div style="display: inline-block; vertical-align: middle; flex: 1">
					<span id="leysen_ozcczx_add_img_upload_span" class="img_thumbnail_add_span">
						<img class="img_upload" id="leysen_ozcczx_add_img_upload" src="/<%=PropKit.get("project_name") %>/res/images/img_add.png">
					</span>
					<span id="leysen_ozcczx_add_img_imgs_div">
					</span>
...
$(document).ready(function(){
	$.CurrentNavtab.find("#leysen_ozcczx_submit").click(function() {
		var imgpath = '';
		$.CurrentNavtab.find('#leysen_ozcczx_add_img_imgs_div img[name="img_thumbnail"]').each(function() {
			imgpath = $(this).attr('src');
		});
		$.CurrentNavtab.find('#imgpath').val(imgpath);
		
		$.CurrentNavtab.find("#leysen_ozcczx_addform").submit();
	});
	
	uploadimg('leysen_ozcczx_add_', 1);
});
