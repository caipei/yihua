...
<form action="/<%=PropKit.get("project_name") %>/admin/brand/ztxl_update/" id="brand_ztxl_addform" data-toggle="validate" data-alertmsg="false">
	<table class="table table-condensed table-hover" width="100%">
        ...
        <tr>
	        <td style="display: flex; align-items: center;">
	            <label class="control-label x100">中图：</label>
	            <div style="display: inline-block; vertical-align: middle; flex: 1">
					<span id="brand_ztxl_edit_zhong_img_upload_span" class="img_thumbnail_add_span">
						<img class="img_upload" id="brand_ztxl_edit_zhong_img_upload" src="/<%=PropKit.get("project_name") %>/res/images/img_add.png">
					</span>
					<span id="brand_ztxl_edit_zhong_img_imgs_div">
					...
					<input id="brand_ztxl_edit_zhong_imgpath" name="brand_ztxl_edit_zhong_imgpath" type="hidden" value="" />
	            </div>
	        </td>
        </tr>
        <tr>
	        <td>
	            <label for="title" class="control-label x100">中图对应产品：</label>
	            <a href="/<%=PropKit.get("project_name") %>/admin/brand/product_choice_dialog?controlname=product_zhong" class= "btn btn-blue" data-toggle="dialog" data-width="600" data-height="400" data-id="dialog_choice_obj" data-mask="true">选择对象</a>
	            <span id="product_zhong_dialog_span">
	            	<a href="/<%=PropKit.get("project_name") %>/brand/product?id=${productsid2 }" class= "btn btn-green" target="_blank">${productsname2 }</a>
	            </span>
	            <input id="product_zhong_dialog_id" name="product_zhong_dialog_id" type="hidden" value="${productsid2 }">
	        </td>
        </tr>
        <tr>
	        <td style="display: flex; align-items: center;">
	            ...
	            <div style="display: inline-block; vertical-align: middle; flex: 1">
					...
					<span id="brand_ztxl_edit_you_img_imgs_div">
						<c:if test="${!empty imgs3}">
						<span class="img_thumbnail_span">
						<img name="img_thumbnail" class="img_thumbnail" src="${imgs3}">
						<img name="img_thumbnail_del" class="img_thumbnail_del" src="/<%=PropKit.get("project_name") %>/res/images/close.png" />
						</span>
						</c:if>
					</span>
					<input id="brand_ztxl_edit_you_imgpath" name="brand_ztxl_edit_you_imgpath" type="hidden" value="" />
	            </div>
	        </td>
        </tr>
        <tr>
	        <td>
	            ...
	            <span id="product_you_dialog_span">
	            	<a href="/<%=PropKit.get("project_name") %>/brand/product?id=${productsid3 }" class= "btn btn-green" target="_blank">${productsname3 }</a>
	            </span>
	            <input id="product_you_dialog_id" name="product_you_dialog_id" type="hidden" value="${productsid3 }">
	        </td>
        </tr>
        </tbody>
    </table>
    ...
    <input id="type" name="type" value="6" type="hidden" />
...
$(document).ready(function(){
	...
	$.CurrentNavtab.find("#brand_ztxl_update").click(function() {
		...
		$.CurrentNavtab.find('#productids').val($('#product_zuo_dialog_id').val() + '|' + $('#product_zhong_dialog_id').val() + '|' + $('#product_you_dialog_id').val());
		...
		$.CurrentNavtab.find('#brand_ztxl_edit_zhong_img_imgs_div img[name="img_thumbnail"]').each(function() {
			imgpath += '|' + $(this).attr('src');
		});
	...
	uploadimg('brand_ztxl_edit_zhong_', 1);
